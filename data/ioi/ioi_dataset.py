...
BABA_TEMPLATES_weird = [
    "Then, [B] and [A] -> [PLACE]. [B] gave a [OBJECT] to [A]",
    "Then, [B] and [A] -> [PLACE]. [B] gave a [OBJECT] to [A]"
]

BABA_TEMPLATES_weird_es = [
    "Luego, [B] y [A] -> [PLACE_es]. [B] dio un/una [OBJECT_es] a [A]",
    "Luego, [B] y [A] -> [PLACE_es]. [B] decidió dar un/una [OBJECT_es] a [A]"
]

BABA_TEMPLATES_weird_fr = [
    "Puis, [B] et [A] -> [PLACE_fr]. [B] a donné un/une [OBJECT_fr] à [A]",
    "Puis, [B] et [A] -> [PLACE_fr]. [B] a décidé de donner un/une [OBJECT_fr] à [A]"
]


BABA_struc = [
    "[B] and [A] -> [PLACE], [B] -> gives -> [OBJECT] -> to [A]", 
    "[B] and [A] -> [PLACE], [B] -> gives -> [OBJECT] -> to [A]"
]
...
def get_end_idxs(prompts, tokenizer, name_tok_len=1, prepend_bos=False, toks=None):
    ...
    for i in range(toks.shape[0]):
        ...
        # print(toks[i])
        # print(end_idxs[i])
        # print(toks[i][end_idxs[i]])
        # print(toks.shape[1])
        # print(toks[i][end_idxs[i] + 2])
...
class IOIDataset:
    def __init__(
        ...
        elif prompt_type == "BABA_weird":
            self.templates = BABA_TEMPLATES_weird[:nb_templates].copy()
        elif prompt_type == "BABA_struc":
            self.templates = BABA_struc[:nb_templates].copy()
        elif prompt_type == "BABA_unstruc":
            self.templates = BABA_unstruc[:nb_templates].copy()
